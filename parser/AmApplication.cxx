...
namespace DCApplication{
	...
	AmApplication::~AmApplication(){
		...
		vector<AmRunnableClass*>::iterator itRunnables;
	...
	vector<dcLabel*> AmApplication::GetLabelsForRunnable(AmRunnableClass* dcRunnableIn){
		...
		for(i=0; i < instructionSize; i++){
			...
			if(instrName == "sw:LabelAccess"){
				...
				dcRemoteAccessInstruction *rinst = static_cast<dcRemoteAccessInstruction*>(inst);
				if(rinst){
	...
	void AmApplication::AddRunnable(AmRunnableClass* RunnableIn){
		Runnables.push_back(RunnableIn);
	}

	void AmApplication::AddRunnable(AmRunnableClass* RunnableIn,string IDIn){
		Runnables.push_back(RunnableIn);
		RunnablesMap[IDIn]=RunnableIn;
	}

	AmRunnableClass* AmApplication::GetRunnable(int Index) {
		return Runnables.at(Index);
	}

	AmRunnableClass* AmApplication::GetRunnable(string IDIn) {
	...
	vector<AmTask*> AmApplication::GetRunnableReleasedTasks(AmRunnableClass* MyRunnable){
	...
	vector<AmRunnableClass*> AmApplication::GetDestinationRunnables(AmRunnableClass* dcRunnableIn){
		vector<AmRunnableClass*> MyRunnables;
	...
	vector<AmRunnableClass*> AmApplication::GetAllRunnable(){
		return Runnables;
	}
...
const map<string, dcCoreType*>& AmApplication::getCoreTypesMap() const {
	return CoreTypesMap;
}
